...
CREATE PROCEDURE [checkmk].[chart_capacity_combined]
WITH ENCRYPTION
AS
BEGIN
    /* This procedure only works with Windows running SQL 2012 and above. */

    /* For every database with files on each drive, this procedure totals up used data+index space, reserved space, and captures drive size.
	...
	BEGIN
        ...
        INSERT INTO #apollosizesmonitoroutput ([drive], [file_size_total_kb])
            SELECT [drive], SUM(file_size_kb) AS "file_sizes" 
        ...
        FROM CTE 
            ...
            INNER JOIN #apollosizesmonitoroutput ao 
                ON ao.[drive] = CTE.[drive]
        WHERE [rownum] = 1;
