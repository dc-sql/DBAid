...
echo -e '#!/bin/sh
find /var/opt/mssql/log/ -name *dbaid*.log -mtime +14 -delete' > /var/opt/mssql/dbaid_delete_system_history.sh
chown mssql:mssql dbaid_delete_system_history.sh
chmod 744 dbaid_delete_system_history.sh
...
(crontab -u mssql -l 2>/dev/null ; echo "0 18 * * * /var/opt/mssql/dbaid_delete_system_history.sh") | awk '!x[$0]++' | crontab -u mssql -
