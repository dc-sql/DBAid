...
BEGIN
	...
	SELECT [class],CAST([value] AS NVARCHAR(260))
	...
	WHERE 
	 [property] = 'StartName'
		AND [value] IN (
			...
			FROM [configg].[wmi_service_property]
			WHERE [class] LIKE '%Win32_GroupUser/Local_Admins%')
				AND ([class] LIKE '%SQLService/MSSQL$%' 
				OR [class] LIKE '%SQLService/MSSQLSERVER%' 
				OR [class] LIKE '%SQLService/SQLAgent%' 
				OR [class] LIKE '%SQLService/SQLSERVERAGENT%'
				OR [class] LIKE '%SQLService/MSSQLFDLauncher%')
	...
	INSERT INTO @results
	SELECT '2.1','2.1 Ad Hoc Distributed Queries Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'ad hoc distributed queries'
	INSERT INTO @results
	SELECT '2.2','2.2 CLR Enabled Server Configuration Option to 0 (Scored) ' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'CLR enabled'
	INSERT INTO @results
	SELECT '2.3','2.3 Set the Cross DB Ownership Chaining Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Cross db ownership chaining'
	INSERT INTO @results
	SELECT '2.4','2.4 Set the Database Mail XPs Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Database Mail XPs'
	INSERT INTO @results
	SELECT '2.5','2.5 Set the Ole Automation Procedures Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Ole Automation Procedures'
	INSERT INTO @results
	SELECT '2.6','2.6 Set the Remote Access Server Configuration Option to 0 (Scored) ' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Remote access'
	INSERT INTO @results
	SELECT '2.7','2.7 Set the Remote Admin Connections Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Remote admin connections'
	INSERT INTO @results
	SELECT '2.8','2.8 Set the Scan For Startup Procs Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'Scan for startup procs'
	INSERT INTO @results
	...
	INSERT INTO @results
	SELECT '2.10','2.10 Disable Unnecessary SQL Server Protocols – ‘Shared Memory and TCP/IP Protocol is enabled’ (Not Scored)' AS [Policy Name] ,CASE WHEN [value] = 'True' THEN 0 ELSE 1 END AS [score], CAST([value] AS NVARCHAR(10)) FROM [configg].[wmi_service_property] WHERE [property] = 'Shared Memory'
	INSERT INTO @results
	SELECT '2.11','2.11 Configure SQL Server to use non-standard ports (Not Scored)' AS [Policy Name], CASE WHEN [value] = '1433' THEN 0 ELSE 1 END AS [score], CAST([value] AS NVARCHAR(10))FROM [configg].[wmi_service_property] WHERE [property] = 'TcpPort'
	INSERT INTO @results
	SELECT '2.12','2.12 Set the Hide Instance option to Yes for Production SQL Server instances (Scored)' AS [Policy Name], CASE WHEN [value] = 'False' THEN 0 ELSE 1 END AS [score], CAST([value] AS NVARCHAR(10)) FROM [configg].[wmi_service_property] WHERE [property] = 'HideInstance'
	INSERT INTO @results
	...
	INSERT INTO @results
	SELECT '2.15','2.15 Set the xp_cmdshell Server Configuration Option to 0 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 0 ELSE 1 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'xp_cmdshell'
	INSERT INTO @results
	...
	INSERT INTO @results
	SELECT '3.1','3.1 Set The Server Authentication Property To Windows Authentication mode (Scored)' AS [Policy Name], CASE WHEN [value] = 0 THEN 0 ELSE 1 END AS [score], CAST([value] AS NVARCHAR(10)) AS [value] FROM [configg].[wmi_service_property] WHERE [property] = 'IsIntegratedSecurityOnly'
	INSERT INTO @results
	...
	IF EXISTS (SELECT [value] FROM @admin_accounts WHERE [class] LIKE '%SQLService/MSSQL$%' OR [class] LIKE '%SQLService/MSSQLSERVER%')
	BEGIN
		INSERT INTO @results
		...
		FROM @admin_accounts
		WHERE 
		  [class] LIKE '%SQLService/MSSQL$%' OR [class] LIKE '%SQLService/MSSQLSERVER%'
	END
	ELSE
	BEGIN
		INSERT INTO @results
	...
	IF EXISTS (SELECT [value] FROM @admin_accounts WHERE [class] LIKE '%SQLService/SQLAgent%' OR [class] LIKE '%SQLService/SQLSERVERAGENT%')
	BEGIN
		INSERT INTO @results
		...
		FROM @admin_accounts
		WHERE 
		  [class] LIKE '%SQLService/SQLAgent%' OR [class] LIKE '%SQLService/SQLSERVERAGENT%'
	END
	ELSE
	BEGIN
		INSERT INTO @results
	...
	IF EXISTS (SELECT [value] FROM @admin_accounts WHERE [class] LIKE '%SQLService/MSSQLFDLauncher%')
	BEGIN
		INSERT INTO @results
		...
		FROM @admin_accounts
		WHERE 
		  [class] LIKE '%SQLService/MSSQLFDLauncher%'
	END
	ELSE
	BEGIN
		INSERT INTO @results
	...
	INSERT INTO @results
	SELECT '5.2', '5.2 Set the Default Trace Enabled Server Configuration Option to 1 (Scored)' AS [Policy Name], CASE [value_in_use] WHEN 1 THEN 1 ELSE 0 END AS [pass], CAST([value_in_use] AS NVARCHAR(10)) AS [value] FROM sys.configurations WHERE [name] = 'default trace enabled'
	INSERT INTO @results
    ...
    INSERT INTO @results
	SELECT '8.1', '8.1 SQL Server Browser Service Disabled (Not Scored)' AS [Policy Name], CASE [value] WHEN '4' THEN 1 ELSE 0 END AS [score], CAST([value] AS NVARCHAR(1)) AS [value] FROM [configg].[wmi_service_property] WHERE [class] LIKE '%SQLBrowser' AND [property] = 'StartMode'
