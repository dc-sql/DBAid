...
CREATE PROCEDURE [configg].[get_instance_replication_publishers]
WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT ON;
	...
	INSERT INTO @db_publication EXEC [system].[usp_execute_foreach_db]  N'SELECT ''?'' FROM [?].[INFORMATION_SCHEMA].[TABLES] [T] INNER JOIN [sys].[databases] [D] ON ''?'' = [D].[name] WHERE [TABLE_NAME]=''syspublications''  AND [D].[is_distributor]=0';
