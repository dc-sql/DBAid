...
CREATE TABLE [checkmk].[tbl_pnp_perfmon_config]
(
    ...
    CONSTRAINT [CK_config_perfcounter_thresholds] 
		CHECK (([check_warning_threshold] IS NULL 
			AND [check_critical_threshold] IS NULL)
				OR ([check_warning_threshold] IS NOT NULL 
					AND [check_critical_threshold] IS NOT NULL))
) 
GO
