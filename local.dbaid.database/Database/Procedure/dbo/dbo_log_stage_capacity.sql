...
CREATE PROCEDURE [dbo].[log_stage_capacity]
WITH ENCRYPTION
AS
...
BEGIN
	...
	SELECT @retention_months=CAST([value] AS TINYINT) FROM [dbo].[static_parameters] WHERE [name] = 'CAPACITY_CACHE_RETENTION_MONTH';

	IF (@retention_months > 0 OR @retention_months IS NULL) SET @retention_months = ISNULL(@retention_months, 3)*-1;
