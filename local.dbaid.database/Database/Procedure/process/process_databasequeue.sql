...
CREATE PROCEDURE [process].[databasequeue]
WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT ON;
		...
		BEGIN TRANSACTION
			...
			BEGIN
				INSERT INTO [audit].[event]([postdate],[message_body],[event_notification]) 
					VALUES(ISNULL(@message_body.value('(/EVENT_INSTANCE/PostTime)[1]','DATETIME'),GETDATE()),@message_body,'DDL_DATABASE_SECURITY_EVENTS');
