...
CREATE PROCEDURE [process].[serverqueue]
WITH ENCRYPTION
AS
BEGIN
	SET NOCOUNT ON;
		...
		BEGIN TRANSACTION;
			...
			BEGIN
				INSERT INTO [audit].[event]([postdate],[message_body],[event_notification]) 
					VALUES(@message_body.value('(/EVENT_INSTANCE/PostTime)[1]','DATETIME'),@message_body,'DDL_SERVER_LEVEL_EVENTS');
